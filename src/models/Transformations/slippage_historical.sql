...
WITH source_data AS (
    ...
    MD5(CONCAT(',', COALESCE(CAST(domain_id AS STRING), ''), COALESCE(CAST(asset AS STRING), ''), 
    COALESCE(CAST(amount AS STRING), ''), COALESCE(CAST(`timestamp` AS STRING), ''))) AS row_hash
